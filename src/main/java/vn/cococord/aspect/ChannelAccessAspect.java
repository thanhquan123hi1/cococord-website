...
public class ChannelAccessAspect {
    ...
    public Object checkChannelAccess(ProceedingJoinPoint joinPoint, CheckChannelAccess checkChannelAccess)
            throws Throwable {
        ...
        // Handle channels without server (e.g. DM channels)
        // If channel has no server, we might skip server-related checks or handle
        // differently
        if (channel.getServer() == null) {
            log.debug("Channel {} has no server (DM?), skipping server member checks", channelId);
            // Proceed as we can't check server membership
            // Note: Verify if this is desired behavior for DMs
